...
keys = [
    ...
    Key([mod], "space", lazy.spawn("rofi -show drun -modes 'drun,run,window'")),
...
# --- Clipboard state ---
clipboard_history = []
clipboard_index = [0]  # use list so lambdas can mutate it
MAX_HISTORY = 20


logo = os.path.join(os.path.dirname("/home/davidvh/Pictures/Wallpapers/"), "shaded.png")
screens = [
    Screen(
        top=bar.Bar(
            [
                ...
                widget.Prompt(),
                widget.Mpd2(
                    status_format='󰝚  {artist} - {title}',
                    idle_format='{idle_message}'
                    ),
                widget.Notify(
                    foreground_urgent="f38ba8",
                    foreground_low="a6adc8",
                    background_urgent="440000",
                    default_timeout=5,
                    max_chars = 30
                    ),
                ...
                widget.Spacer(length=bar.STRETCH),
                widget.Wttr(
                    location={"Glenmarais":"Glenmarais"},
                    format="%l: %c%t", 
                    update_interval=600
                    ),
                widget.ThermalSensor(format=' {temp:.1f}{unit}'),
                widget.CPU(format='   {load_percent}%'),
                widget.Memory(format='󰍛  {MemUsed:.0f}G/{MemTotal:.0f}G', measure_mem='G'),
                widget.NvidiaSensors(format='GPU  {perf} | {temp}°C'),
                ...
                widget.Battery(
                    ...
                    empty_char = '󰁺',
                    unknown_char = '󰂑',
                    format = '{char} {percent:2.0%} {hour:d}h:{min:02d} left',
                    charge_controller=lambda: (0,85)
                    )
            ],
            24,
            margin=[0,0,5,0],
...
wl_input_rules = {
    "type:touchpad": InputConfig(
        tap=True,
        natural_scroll=True  # Natural direction
    ),
}
